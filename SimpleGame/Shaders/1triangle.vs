...
void Falling5(){
	float newTime = u_Time-a_Vel.z;
	if(newTime>0){
	float lifetime= (a_Vel.w+0.5)*0.2;
	//시험이라면 mod()라고 줄 예정
	float t = mod(newTime , lifetime); 
	//여기도 빈칸주고 어떻게 하면 라이프타임이 끝나면 사라지게 할거임
	float scale = a_Vel.z*((lifetime-t)/lifetime);
	float vx = a_Vel.x;
	float vy = a_Vel.y;
    float initPosX=a_Position.x*a_Vel.z+cos(vx*c_PI*2);
	float initPosY=a_Position.y*a_Vel.z+sin(vx*c_PI*2);
...
*/
//문제가 처음에는 작았다가 점점커져서 진폭이 주기는 바꾸지말고
//다른 문제 그리고 한점에 모아지도록
//다른문제 처음 시작에서 두배가 커지도록

void Thrust(){
	float newTime = u_Time-r_1;

	if(newTime>0){
	float t = mod(newTime*2,2.0); //0~1
	//float ampScale = 0.5-0.5*t;//이게 한점에 모아지는것
	float ampScale = 2*t;
	float amp = (r_1)*2*ampScale;
	vec4 newPosition;
	//문제중에 맨 왼쪽에서 시작을 한다라는 문제가 있다면
	//왼쪽으로도 하고 주기도 끝나지 않게 만들어야함
	newPosition.x = a_Position.x+2*t-1.0;
	newPosition.y = a_Position.y + sin(t * 3.141592 * 2.0*r_1)*amp;
	newPosition.z = a_Position.z;
	gl_Position = newPosition;
	}
...
//사인위치에 동일하게 움직이는데 폭을 바꿔서 폭을 바꿔서 여러개가 되도록
